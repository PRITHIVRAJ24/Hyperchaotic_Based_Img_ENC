import cv2
import numpy as np
from scipy.integrate import solve_ivp
import os
import pickle


def chen_system(t, state, a=36, b=3, c=28, d=0.7, r=0.5):
    x, y, z, w = state
    return np.array([
        a * (y - x),
        (c - a) * x - x * z + c * y,
        x * y - b * z,
        -d * z + r * w
    ], dtype=np.float64)

def generate_chaotic_sequence(length, initial_conditions=[0.1, 0.2, 0.3, 0.4]):
    t_span = (0, length * 0.01)
    t_eval = np.linspace(*t_span, length)
    sol = solve_ivp(chen_system, t_span, initial_conditions, t_eval=t_eval, method='RK45')
    return np.mod(np.abs(sol.y[0]), 1)

def permute_pixels(img, key_seq):
    rows, cols, channels = img.shape
    flat_img = img.reshape(-1, 3)
    permuted_idx = np.argsort(key_seq[:rows * cols])
    
    permuted_img = np.zeros_like(flat_img)
    permuted_img[permuted_idx] = flat_img
    
    return permuted_img.reshape(img.shape), permuted_idx

def xor_diffuse(img, key_seq):
    flat_img = img.flatten()
    key_seq = (key_seq[:flat_img.size] * 256).astype(np.uint8)
    key_seq = np.resize(key_seq, flat_img.shape)  # Ensure key matches image size
    diffused = flat_img ^ key_seq
    return diffused.reshape(img.shape)

def inverse_xor_diffuse(img, key_seq):
    flat_img = img.flatten()
    key_seq = (key_seq[:flat_img.size] * 256).astype(np.uint8)
    key_seq = np.resize(key_seq, flat_img.shape)  # Ensure key matches image size
    diffused = flat_img ^ key_seq
    return diffused.reshape(img.shape)

def inverse_permute_pixels(img, permuted_idx):
    rows, cols, channels = img.shape
    flat_img = img.reshape(-1, 3)
    original_img = np.zeros_like(flat_img)
    original_img[permuted_idx] = flat_img
    return original_img.reshape(img.shape)

def mse(imageA, imageB):
    err = np.sum((imageA.astype("float") - imageB.astype("float")) ** 2)
    err /= float(imageA.shape[0] * imageA.shape[1] * imageA.shape[2])
    return err

def psnr(imageA, imageB):
    mse_value = mse(imageA, imageB)
    if mse_value == 0:
        return 100  
    return 10 * np.log10((255 ** 2) / mse_value)

def encrypt_image(img_path, save_path):
    if not os.path.exists(img_path):
        print(f"Error: Input image '{img_path}' not found.")
        return
    img = cv2.imread(img_path)
    if img is None:
        print("Error: Failed to read the input image.")
        return
    
    rows, cols, channels = img.shape
    key_seq = generate_chaotic_sequence(rows * cols * channels)
    key_seq = np.resize(key_seq, (rows * cols * channels))  # Ensure correct length
    
    permuted_img, permuted_idx = permute_pixels(img, key_seq[:rows * cols])
    diffused_img = np.zeros_like(permuted_img)
    
    for i in range(3):
        diffused_img[:, :, i] = xor_diffuse(permuted_img[:, :, i], key_seq[i::3])
    
    metadata = {"permuted_idx": permuted_idx.tolist()}
    with open(save_path + ".meta", "wb") as f:
        pickle.dump(metadata, f)
    
    cv2.imwrite(save_path, diffused_img)
    enc_mse = mse(img, diffused_img)
    enc_psnr = psnr(img, diffused_img)
    print(f"MSE: {enc_mse}, PSNR: {enc_psnr}")
    print(f"Encrypted image saved at: {os.path.abspath(save_path)}")

def decrypt_image(encrypted_path, save_path):
    if not os.path.exists(encrypted_path):
        print(f"Error: Encrypted image '{encrypted_path}' not found.")
        return
    encrypted_img = cv2.imread(encrypted_path)
    if encrypted_img is None:
        print("Error: Failed to read the encrypted image.")
        return
    
    meta_path = encrypted_path + ".meta"
    if not os.path.exists(meta_path):
        print("Error: Missing metadata file for decryption.")
        return
    
    with open(meta_path, "rb") as f:
        metadata = pickle.load(f)
    permuted_idx = np.array(metadata["permuted_idx"])
    
    rows, cols, channels = encrypted_img.shape
    key_seq = generate_chaotic_sequence(rows * cols * channels)
    key_seq = np.resize(key_seq, (rows * cols * channels))  # Ensure correct length
    
    diffused_img = np.zeros_like(encrypted_img)
    for i in range(3):
        diffused_img[:, :, i] = inverse_xor_diffuse(encrypted_img[:, :, i], key_seq[i::3])
    
    original_img = inverse_permute_pixels(diffused_img, permuted_idx)
    cv2.imwrite(save_path, original_img)
    print(f"Decrypted image saved at: {os.path.abspath(save_path)}")

encrypt_image("application/src/Sample/Lenna.png", "encrypted.png")
decrypt_image("encrypted.png", "decrypted.png")